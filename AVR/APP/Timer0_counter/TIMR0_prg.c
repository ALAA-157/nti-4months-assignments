...
void TIMER0_void_Init(void)
{
		...
		CLEAR_BIT(TCCR0 , 1);
		SET_BIT(TCCR0 , 2);
	#elif(TIMER0_PRESCALER == EXTERNAL_CLOCK_RISING)
		SET_BIT(TCCR0 , 0);
		SET_BIT(TCCR0 , 1);
		SET_BIT(TCCR0 , 2);
	#elif(TIMER0_PRESCALER == EXTERNAL_CLOCK_FALLING)
		CLEAR_BIT(TCCR0 , 0);
		SET_BIT(TCCR0 , 1);
		SET_BIT(TCCR0 , 2);
		...
		SET_BIT(TCCR0 , 4)
		CLEAR_BIT(TCCR0 , 5);
	#elif((TIMER0_COM_EVENT == TIMER0_CLEAR)||(TIMER0_COM_EVENT == TIMER0_INVERTING) )
		CLEAR_BIT(TCCR0 , 4);
		SET_BIT(TCCR0 , 5);
	#elif((TIMER0_COM_EVENT == TIMER0_SET)||(TIMER0_COM_EVENT == TIMER0_NIN_INVERTING) )
		SET_BIT(TCCR0 , 4);
		SET_BIT(TCCR0 , 5);
